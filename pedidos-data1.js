...
/* --- Función para inyectar el selector de operadora --- */
function injectContactEnhancements() {
  ...
  if (phoneInput && !document.getElementById('cust_operator')) {
    ...
    // Estilos directos para asegurar que se vea bien
    ...
    // Opciones de Venezuela
    const operators = ['0412', '0414', '0416', '0424', '0426', '0212'];
    ...
    // Contenedor flex para alinear select + input
    ...
    // Ajustar el input de número
    phoneInput.style.flex = '1';
    phoneInput.placeholder = '1234567';
  }
}

function renderCartPanel() {
  const container = document.getElementById('cartPanel');
  if (!container) return;
  ...
  if (!items.length) {
    ...
    for (const it of items) {
           ...
           <div class="qty-controls" style="justify-content:flex-start;gap:0.35rem;margin:0.6rem 0 0 0;">
             ...
             <button class="btn-secondary remove-item" data-id="${it.productId}" style="margin-left:18px;border-radius:9px;">
              ...
              class="bi bi-trash" viewBox="0 0 16 16">
                   ...
                   <path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a 1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z"/>
               </svg>
             </button>
      ...
      `;
      // Vincular eventos (Cerrar, Vaciar, etc)
      attachGlobalEvents();

      // Actualizar contador en la UI
      renderCartCount();

      // EJECUCIÓN CRÍTICA: Inyectar el selector de operadora 
      // Se usa un pequeño retraso para asegurar que el DOM esté listo
      setTimeout(() => {
        injectContactEnhancements();
      }, 50);
...
function setupGeolocationButton() {
  ...
  geoBtn.addEventListener('click', async () => {
    ...
    if (location.protocol !== 'https:' && location.hostname !== 'localhost') {
      geoError('La geolocalización requiere conexión segura (HTTPS).');
      return;
    }
    ...
    try {
      ...
      // Convertir a dirección con Google Geocoding API
      ...
      if (data.status === "OK" && data.results[0]) {
        ...
        throw new Error('No se obtuvo dirección válida');
      }
    } catch (error) {
      console.error('Error en geolocalización:', error);
      if (error.code === 1) {
        geoError('Debes permitir el acceso a tu ubicación.');
      } else if (error.code === 2 || error.code === 3) {
        geoError('No se pudo conectar al servicio de ubicación.');
      } else {
        geoError('No se pudo obtener tu ubicación precisa. Ingresa la dirección manualmente.');
      }
      showGeoButton(); // Permitir reintento o entrada manual
    } finally {
